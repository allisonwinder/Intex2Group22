...
public class CspMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        context.Response.Headers.Add("Content-Security-Policy", $"default-src 'self' 'unsafe-inline'; script-src 'self' 'nonce-{nonceValue}' ; style-src  'self' 'unsafe-inline'; font-src 'self'; img-src 'self'; frame-src 'self'");
        await _next(context);
    }
}
